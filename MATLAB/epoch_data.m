...
%   mode ('lead') - Can be 'lead' or 'lag'. If lead, excludes leading
...
% Validate epochs
if size(epochs,2)~=2||any(epochs(:,2)>epochs(:,1))||any(arrayfun(@(t)any(t>epochs(:,1)&t<epochs(:,2)),epochs(:,1)))||any(arrayfun(@(t)any(t>epochs(:,1)&t<epochs(:,2)),epochs(:,2)))
    error('epoch_data:badEpochs','Error. Epochs must be a matrix of size mX2 as a list of non-overlapping epoch starts and stops.');
end

% Import data
ip = inputParser;
ip.addParamValue('mode','lead');
...
% Format lead_epochs based on mode
if isequal(lower(mode),'lead')
    lead_epochs = [epochs(:,1) epochs(:,2)-max_lag];
elseif iequal(lower(mode),'lag')
    lead_epochs = epochs;
else
    error('epoch_data:unrecognizedMode','Error. Unrecognized mode %s. Must be ''lead'' or ''lag''',mode);
end
